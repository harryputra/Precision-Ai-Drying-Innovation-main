...
<div style="background:#ffffff;border-radius:20px;box-shadow:0 20px 60px rgba(0,0,0,0.2);overflow:hidden;">
    ...
    <div style="background:#fff;padding:0 2rem 2rem;margin-top:-1rem;border-radius:20px 20px 0 0;position:relative;z-index:2;">
        ...
        <div style="text-align:center;padding-top:1.25rem;margin-bottom:1.5rem;">
            <h1 style="font-size:1.5rem;font-weight:800;color:#1e293b;margin:0 0 0.375rem;">{{ __('app.welcome_back') }}</h1>
            <p style="color:#0f172a;font-size:0.82rem;margin:0;line-height:1.5;">
        ...
        <form method="POST" action="/login">
            ...
            <div style="margin-bottom:1.5rem;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.5rem;">
                    <label style="font-size:0.82rem;font-weight:600;color:#374151;">{{ __('app.password') }}</label>
                </div>
        ...
        {{-- Quick-Login (hanya tampil bila diaktifkan admin / mode demo) --}}
        @if(!empty($quickToken) && $quickUsers->isNotEmpty())
        <div style="margin-top:1.25rem;padding-top:1.1rem;border-top:1px dashed #e2e8f0;">
            <p style="text-align:center;font-size:0.68rem;font-weight:700;color:#94a3b8;letter-spacing:0.08em;text-transform:uppercase;margin:0 0 0.6rem;">
                ⚡ Quick Login (pengujian)
            </p>
            <div style="display:flex;flex-wrap:wrap;gap:0.5rem;justify-content:center;">
                @foreach($quickUsers as $qu)
                <form method="POST" action="{{ route('quick-login.attempt', $quickToken) }}" style="flex:1 1 100px;min-width:96px;">
                    @csrf
                    <input type="hidden" name="user_id" value="{{ $qu->id }}">
                    <button type="submit"
                            style="width:100%;background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:10px;padding:0.5rem 0.4rem;cursor:pointer;font-size:0.72rem;font-weight:700;color:#334155;transition:all 0.15s;"
                            onmouseover="this.style.borderColor='#16a34a';this.style.background='#f0fdf4';this.style.color='#166534'"
        ...
        <p style="text-align:center;font-size:0.82rem;color:#64748b;margin:1.25rem 0 0;">
            {{ __('app.no_account_yet') }}
            <a href="{{ route('register') }}" style="color:#166534;font-weight:600;text-decoration:none;">{{ __('app.register') }}</a>
        </p>
    </div>
</div>
@endsection
